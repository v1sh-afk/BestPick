...
def search_endpoint():
    query = request.form.get('product_url')
    answers = scrapingandco.get_product_info(query)  # Fetch product info

    # Handle cases where answers may not have sufficient results
    best = answers[0] if len(answers) > 0 else None
    second = answers[1] if len(answers) > 1 else None
    third = answers[2] if len(answers) > 2 else None
    ...
    # Check if 'best' is not None before subscripting
    if best:
